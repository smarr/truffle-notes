...
FrameSlot getFrameSlot(final LocalReference reference) {
...
```java
public ExpressionNode visitAssignmentStatement(AssignmentStatement assignmentStatement) {
  LocalReference reference = assignmentStatement.getLocalReference();
  FrameSlot slot = getFrameSlot(reference);
  return LocalVariableWriteNodeGen.create(
      slot, (ExpressionNode) assignmentStatement.
...
abstract class LocalVariableWriteNode extends ExpressionNode {
  ...
  @Specialization
  public final Object writeGeneric(VirtualFrame frame, Object expValue) {
    ensureObjectKind();
...
Golo got multiple types of loops including classic `for` and `while` loops.
...
post-condition. We translate this construct to the following node implementation:
...
expression. In a classic `for` loop, it could for instance initialize an
induction variable `i` with an initial value. The second child node is
Truffle's `LoopNode`, which implements general iteration with support for
...
The actual loop execution happens in the `LoopNode`. From the language
implementer perspective, the `LoopNode` requires an implementation of a
`RepeatingNode` that implements a single method which does a loop iteration
and indicates whether it should continue.
