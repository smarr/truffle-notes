...
In the the [previous post][0] of this series, we built up all the
...
times_truf_ns <- c(170372, 174281, 170803, 169305, 173985, 165851, 172163, 164394, 171147, 172138, 169704)
...
`r round(g/t,1)`x faster than the bytecode-based version. And only about
`r round(t/j,1)`x slower than the pure Java benchmark.

In the next post, we'll try to close the gap to Java a little more by
investigating how the compilation works, and removing one source for overhead
we have not optimized for yet.
...
[0]: http://previous-post-todo
[1]: http://first-post-todo
[2]: http://previous-post-todo#arg-reads
[3]: http://previous-post-todo#func-invocation
