...
## What Is Truffle and How Can It Be Useful?

[Truffle][1] is a Java framework for implementing interpreters. In combination
with [Graal][2], it provides just-in-time (JIT) compilation based on simple
[abstract-syntax-tree (AST) interpreters][3]. This means, one only has to
implement the language once as part of the AST, and essentially gets a JIT
compiler that can reach the performance of state-of-the-art virtual machines
such as HotSpot for Java or V8 for JavaScript. Thus, Truffle is a useful
approach for implementing languages that need to reach good performance. An
alternative approach would be [RPython][4] with its meta-tracing just-in-time
compiler. A more thorough comparison of the two approaches can be found
[here][5] and [here][6]. However, for this series of blog posts, we are
interested in a language running on top of the JVM, and thus, Truffle is our
framework of choice.

# Step 1: Setting A Goal and Choose A Benchmark

While Truffle provides a nice set of features to simplify the implementation of
languages, often the desire for better performance is a major reason for
adopting it. Thus, we will thus focus on using Truffle to improve the
performance of a language implementation.

## Improve the Numerical Performance of Golo
...
## Use Mandelbrot as a Benchmark

To keep it simple, but still chose something that can be used to investigate
interesting performance aspects, we will take a Mandelbrot calculation as our
main example. The following code snippet is an abbreviated version of the [Java
benchmark][9] we will use:
...
The first property to notice is that the computation is inside of three nested
loops. Furthermore, most operations are on `double` values. For a dynamic
language, this means that optimization of the lookup of operators and methods
as well as the way `double` values are treated is essential for performance.
...
## Oops, Golo Doesn't Have Bit Operations

Ah well, before I forget it: Golo does not actually have bit operations. So,
before we truffle-ize it, the next part of this blog post series will look
briefly into the current Golo implementation. It compiles Golo directly to
JVM bytecodes and relies heavily on `invokedynamic` to realize the dynamic
language semantics. By adding bit operations, we will get a basic idea of
how the implementation works to be able to compare it later to the Truffle
version.
