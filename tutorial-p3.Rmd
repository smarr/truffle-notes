...
purposes we add `ExpressionNode`,<sup>1</sup><span class="sidenote"><sup>1</sup>
...
avoid having different function invocation nodes for different number of
arguments, we introduce a `EvalArgumentsNode` that will create an array of
...
advanced use case and requires extra care. Normally, the DSL shields us from
...
such safeguards, we have to manually make sure that a node is only used once
...
calls, and communicate this knowledge to the compiler.<sup>3</sup><span
class="sidenote"><sup>3</sup> They can also
...
perform inlining, because it would most likely down execution.
...
Evaluating the condition is actually a little more complex.<sup>4</sup><span
class="sidenote"><sup>4</sup> I refrained 
...
quite a bit of a different mind set. When generating bytecodes, one has to be
...
what we want to do, and than can codify it in *normal* Java. We do not have to
...
### Truffle Concepts, Conventions, and Other Important Bits
 ...
 - `@Children` ([sec. 4.1](#sequences)), marks subnodes to allow framework to
   do correct node replacement in AST, and implicitly compilation final
